            ...
            {activeTab === 'squad' ? (
              <div className="flex items-center">
                <div className="flex gap-2 mr-4">
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-vista-secondary/40">
                    <CheckCircleIcon className="h-3 w-3 text-green-400" />
                    <span className="text-xs">{players.filter(p => !p.status || p.status === 'READY').length}</span>
                  </div>
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-vista-secondary/40">
                    <HeartIcon className="h-3 w-3 text-yellow-400" />
                    <span className="text-xs">{players.filter(p => p.status === 'REHABILITATION').length}</span>
                  </div>
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-vista-secondary/40">
                    <ExclamationCircleIcon className="h-3 w-3 text-red-400" />
                    <span className="text-xs">{players.filter(p => p.status === 'SICK').length}</span>
                  </div>
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-vista-secondary/40">
                    <AcademicCapIcon className="h-3 w-3 text-blue-400" />
                    <span className="text-xs">{players.filter(p => p.status === 'STUDY').length}</span>
                  </div>
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-vista-secondary/40">
                    <QuestionMarkCircleIcon className="h-3 w-3 text-gray-400" />
                    <span className="text-xs">{players.filter(p => p.status === 'OTHER').length}</span>
                  </div>
                </div>
                <Dialog open={showAddPlayerDialog} onOpenChange={setShowAddPlayerDialog}>
                  <DialogTrigger asChild>
                    <Button variant="default" className="ml-2 bg-vista-primary text-vista-dark hover:bg-vista-primary/90 text-xs px-3 py-0.5 h-7">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
            {players.map(player => (
              <Link 
                ...
                className="bg-vista-secondary/50 rounded-lg overflow-hidden hover:bg-vista-secondary/70 transition-colors relative border border-t-vista-light/30 border-r-vista-primary/50 border-l-vista-primary/50 border-b-vista-primary/80"
