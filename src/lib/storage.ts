...
export const getClubStoragePath = (clubId: string) => {
...
export const getExercisesStoragePath = (clubId: string) => {
...
export const getExerciseStoragePath = (clubId: string, exerciseId: string) => {
  const exercisePath = path.join(getExercisesStoragePath(clubId), exerciseId);
  ensureDirectoryExists(exercisePath);
  return exercisePath;
};

// Типизация для File, которая может отсутствовать в NodeJS окружении
...
export const saveExerciseFile = async (
  clubId: string,
  exerciseId: string,
  file: Buffer | FileWithArrayBuffer,
  filename: string
) => {
  try {
...
export const deleteExerciseDirectory = (clubId: string, exerciseId: string) => {
  try {
    const exercisePath = getExerciseStoragePath(clubId, exerciseId);
