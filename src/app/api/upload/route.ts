...
export async function POST(req: NextRequest) {
  try {
    ...
    // Проверка типа файла в зависимости от параметра fileType
    if (fileType === 'exercise') {
      // Для упражнений разрешаем изображения и видео
      if (!file.type.startsWith('image/') && !file.type.startsWith('video/')) {
        return NextResponse.json({ error: 'Для упражнений можно загружать только изображения или видео' }, { status: 400 });
      }
    } else if (fileType === 'document') {
      // Для документов разрешаем изображения и PDF
    if (!file.type.startsWith('image/') && !file.type.startsWith('application/pdf')) {
        return NextResponse.json({ error: 'Для документов можно загружать только изображения или PDF-файлы' }, { status: 400 });
      }
    } else {
      // По умолчанию (для фото и других типов) разрешаем только изображения
    ...
    try {
      ...
      if (bucketError) {
        ...
        if (bucketError.message && bucketError.message.includes('not found')) {
          ...
          const { error: createError } = await supabaseAdmin.storage.createBucket('vista-media', {
            public: true
          });
    ...
    try {
      ...
      if (folderError) {
        ...
        const { error: folderCreateError } = await supabaseAdmin.storage
          .from('vista-media')
