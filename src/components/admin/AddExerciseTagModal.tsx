...
export const AddExerciseTagModal: React.FC<AddExerciseTagModalProps> = ({ open, onOpenChange, newTag, onChange, onAdd, onCancel, error, loading }) => (
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="bg-vista-dark/95 border border-vista-secondary/30 text-vista-light shadow-xl rounded-xl max-w-md w-full overflow-hidden backdrop-blur-xl">
      <DialogHeader>
        <DialogTitle className="text-lg font-semibold text-center mb-2 text-vista-light">Добавить тег упражнения</DialogTitle>
      </DialogHeader>
      {error && (
      ...
      <div className="space-y-4 py-2">
        <div className="space-y-2">
          ...
          <Input id="exercise-tag-name" name="name" value={newTag.name} onChange={onChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light" placeholder="Введите название" disabled={loading} />
        </div>
      </div>
