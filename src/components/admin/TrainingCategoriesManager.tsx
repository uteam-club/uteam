...
export function TrainingCategoriesManager() {
  ...
  const [currentCategoryToDelete, setCurrentCategoryToDelete] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        console.error('Ошибка загрузки категорий тренировок:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t('errorAddingCategory'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditCategory = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t('errorEditingCategory'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/training-categories?id=${id}`, {
        method: 'DELETE',
      });
      ...
      setCurrentCategoryToDelete(null);
    } catch (error) {
  ...
  const openDeleteDialog = (categoryId: string) => {
    setCurrentCategoryToDelete(categoryId);
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-vista-light">
          {t('trainingCategoriesManagement')}
        </h2>
          ...
          <div className="py-4 text-vista-light">
            {currentCategoryToDelete && (
              t('categoryDeleteConfirmation', { 
                name: categories.find(c => c.id === currentCategoryToDelete)?.name || ''
              })
            )}
          </div>
          <DialogFooter>
            ...
            <Button 
              variant="destructive" 
              onClick={() => currentCategoryToDelete && handleDeleteCategory(currentCategoryToDelete)}
              disabled={loading}
