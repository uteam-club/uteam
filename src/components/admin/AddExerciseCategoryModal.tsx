...
interface AddExerciseCategoryModalProps {
  open: boolean;
...
export const AddExerciseCategoryModal: React.FC<AddExerciseCategoryModalProps> = ({ open, onOpenChange, newCategory, onChange, onAdd, onCancel, error, loading }) => (
  <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="bg-vista-dark/95 border border-vista-secondary/30 text-vista-light shadow-xl rounded-xl max-w-md w-full overflow-hidden backdrop-blur-xl">
      <DialogHeader>
        <DialogTitle className="text-lg font-semibold text-center mb-2 text-vista-light">Добавить категорию упражнения</DialogTitle>
      </DialogHeader>
      {error && (
