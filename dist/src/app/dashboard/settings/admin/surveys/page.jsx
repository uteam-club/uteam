...
export default function SurveysPage() {
    ...
    const handleOpenSurvey = async () => {
        console.log('Клик по кнопке Открыть', club === null || club === void 0 ? void 0 : club.id);
        if (!(club === null || club === void 0 ? void 0 : club.id))
            return;
        setLoading(true);
        try {
            const res = await fetch(`/api/survey/active-id?tenantId=${club.id}`);
            if (!res.ok)
                throw new Error('Не найден активный опросник');
            const data = await res.json();
            if (!data.surveyId)
                throw new Error('Нет surveyId');
        ...
        <Card className="p-6 bg-vista-dark/50 border-vista-secondary/50">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-vista-dark/30 rounded-lg">
              ...
              <a href={`/dashboard/survey/4965979b-0185-4e87-983e-95efa0b5a3b5?tenantId=4ce0dab3-9421-4669-9cd8-cc63ed9d9d63`} className="ml-4 underline text-vista-primary" target="_blank" rel="noopener noreferrer">
                Открыть (тестовая ссылка)
              </a>
            </div>
          </div>
        </Card>
        <button onClick={handleOpenSurvey} className="bg-vista-primary hover:bg-vista-primary/90 px-4 py-2 rounded text-white font-semibold" disabled={loading}>
