...
import { X, Check, ChevronDown, Search, Plus, Upload, Play, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useUsers, useCategories, useTags, useFilteredExercises } from '@/hooks/useExerciseData';
...
export default function ExercisesPage() {
    ...
    const [filePreviewEdit, setFilePreviewEdit] = useState(null);
    ...
    const filteredTags = useMemo(() => {
        return newExercise.categoryId
            ? tagsData.filter((tag) => tag.exerciseCategoryId === newExercise.categoryId)
            : [];
    }, [newExercise.categoryId, tagsData]);
    // Обработчик изменения полей формы редактирования
    const handleEditInputChange = (e) => {
        const { name, value } = e.target;
        setEditExerciseForm((prev) => (Object.assign(Object.assign({}, prev), { [name]: value })));
        // Очистка ошибки при вводе
        if (name in editErrors) {
            setEditErrors((prev) => (Object.assign(Object.assign({}, prev), { [name]: undefined })));
        }
    };
    // Обработчик выбора категории для редактирования
    const handleEditCategoryChange = (value) => {
        setEditExerciseForm((prev) => (Object.assign(Object.assign({}, prev), { categoryId: value, 
            // Сбрасываем теги при смене категории
            tags: [] })));
        // Очистка ошибки при выборе
        setEditErrors((prev) => (Object.assign(Object.assign({}, prev), { categoryId: undefined })));
    };
    ...
    const handleEditTagToggle = (tagId) => {
        setEditExerciseForm((prev) => {
            ...
            return Object.assign(Object.assign({}, prev), { tags: isSelected
    ...
    const handleEditFileChange = (e) => {
        ...
        setEditExerciseForm((prev) => (Object.assign(Object.assign({}, prev), { file })));
        if (file) {
            const reader = new FileReader();
            reader.onloadend = () => {
                setFilePreviewEdit(reader.result);
            };
    ...
    const startEditExercise = () => {
        ...
        setEditExerciseForm({
            ...
            length: ((_a = previewExercise.length) === null || _a === void 0 ? void 0 : _a.toString()) || '',
            width: ((_b = previewExercise.width) === null || _b === void 0 ? void 0 : _b.toString()) || '',
    ...
    const saveEditedExercise = async () => {
        if (!previewExercise || !validateEditForm())
            return;
        try {
            // Сохраняем оригинальные данные для возможного отката
            const originalExercise = Object.assign({}, previewExercise);
            ...
            const optimisticExercise = Object.assign(Object.assign({}, previewExercise), { title: editExerciseForm.title, description: editExerciseForm.description, categoryId: editExerciseForm.categoryId, category: categoriesData.find((c) => c.id === editExerciseForm.categoryId) || previewExercise.category, tags: editExerciseForm.tags.map(tagId => tagsData.find((t) => t.id === tagId)).filter(Boolean), length: editExerciseForm.length ? parseFloat(editExerciseForm.length) : previewExercise.length, width: editExerciseForm.width ? parseFloat(editExerciseForm.width) : previewExercise.width });
            ...
            mutate((key) => typeof key === 'string' && key.startsWith('/api/exercises/filter'), (currentData) => {
                if (!currentData)
                    return currentData;
                return Object.assign(Object.assign({}, currentData), { exercises: currentData.exercises.map((ex) => ex.id === previewExercise.id ? optimisticExercise : ex) });
            ...
            const filterUrl = `/api/exercises/filter?${new URLSearchParams(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, (searchQuery ? { search: searchQuery } : {})), (selectedAuthor ? { authorId: selectedAuthor } : {})), (selectedCategory ? { categoryId: selectedCategory } : {})), (selectedTags.length > 0 ? { tags: selectedTags.join(',') } : {})), { page: currentPage.toString(), limit: '20', t: timestamp.toString() })).toString()}`;
            ...
            setPreviewExercise(prevState => {
                if (prevState) {
                    ...
                    return Object.assign(Object.assign({}, updatedExercise), { 
                        // Добавляем данные, которые могут отсутствовать в ответе API
                        author: prevState.author, category: categoriesData.find((c) => c.id === updatedExercise.categoryId) || prevState.category, tags: updatedExercise.tags || prevState.tags });
                }
                return null;
            });
        }
    ...
    const deleteExercise = async () => {
        ...
        try {
            ...
            mutate((key) => typeof key === 'string' && key.startsWith('/api/exercises/filter'), (currentData) => {
                if (!currentData)
                    return currentData;
                return Object.assign(Object.assign({}, currentData), { exercises: currentData.exercises.filter((ex) => ex.id !== exerciseIdToDelete) });
            }, false // не делаем ревалидацию сразу
            );
    ...
    const filteredEditTags = useMemo(() => {
        return editExerciseForm.categoryId
            ? tagsData.filter((tag) => tag.exerciseCategoryId === editExerciseForm.categoryId)
            : [];
    ...
    const TagsSection = () => {
        ...
        if (isTagsError) {
          ...
          <Button variant="outline" size="sm" onClick={handleTagsRefresh} className="flex items-center">
        ...
        if (!tagsData || tagsData.length === 0) {
            return (<div className="text-center p-4 text-vista-secondary">
          Нет доступных тегов
        </div>);
        }
        return (<div className="max-h-[300px] overflow-y-auto space-y-1 pr-1">
        {tagsData.map((tag) => {
                const isSelected = selectedTags.includes(tag.id);
                return (<div key={tag.id} className={`flex items-center space-x-2 rounded-md p-2 cursor-pointer hover:bg-vista-secondary/20 ${isSelected ? 'bg-vista-primary/20' : ''}`} onClick={() => {
              ...
              <div onClick={(e) => {
                        ...
                        setSelectedTags(isSelected
                            ? selectedTags.filter(id => id !== tag.id)
                            : [...selectedTags, tag.id]);
                    }} className={`flex h-4 w-4 items-center justify-center rounded-sm border ${isSelected
    ...
    return (<div className="space-y-6">
      <Card className="bg-vista-dark/30 border-vista-secondary/30">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={() => setIsCreateDialogOpen(true)} className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
        ...
        <CardContent>
          ...
          <div className="mb-6 space-y-4">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="relative flex-1">
                ...
                <Input placeholder="Поиск упражнений..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-vista-dark/70 border-vista-secondary/30 text-vista-light focus:border-vista-primary"/>
                {searchQuery && (<button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-vista-light/50 hover:text-vista-light">
                    <X className="h-4 w-4"/>
                  </button>)}
              </div>
              
              {/* Фильтр по автору */}
              <Select value={selectedAuthor === null ? 'all' : selectedAuthor} onValueChange={(value) => setSelectedAuthor(value === "all" ? null : value)}>
              ...
              {/* Фильтр по категории */}
              <Select value={selectedCategory === null ? 'all' : selectedCategory} onValueChange={(value) => setSelectedCategory(value === "all" ? null : value)}>
                <SelectTrigger className="w-full sm:w-[200px] bg-vista-dark/70 border-vista-secondary/30 text-vista-light">
                  <SelectValue placeholder="Выберите категорию"/>
                </SelectTrigger>
                <SelectContent className="bg-vista-dark border-vista-secondary/30 text-vista-light shadow-lg backdrop-blur-lg">
                  <SelectItem value="all">Все категории</SelectItem>
                  {categoriesData.map((category) => (<SelectItem key={category.id} value={category.id}>
                      {category.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
              ...
              <div className="relative">
                <Button variant="outline" className="w-full sm:w-[200px] justify-between bg-vista-dark/70 border-vista-secondary/30 text-vista-light" onClick={() => setTagsPopoverOpen(!tagsPopoverOpen)}>
                  {selectedTags.length > 0 ? `${selectedTags.length} тегов` : "Выберите теги"}
                  <ChevronDown className="ml-2 h-4 w-4 shrink-0 opacity-50"/>
                </Button>
                
                {tagsPopoverOpen && (<div className="absolute z-50 w-full sm:w-[250px] mt-1 bg-vista-dark border border-vista-secondary/30 rounded-md shadow-lg">
                    <div className="p-2">
                      <div className="relative mb-2">
                        <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-vista-light/50"/>
                        <Input placeholder="Поиск тега..." className="pl-8 pr-2 h-9 text-vista-light bg-vista-dark/30 border-vista-secondary/30"/>
                      </div>
                      ...
                      <div className="flex justify-end mt-2 pt-2 border-t border-vista-secondary/30">
                        <Button variant="outline" size="sm" className="mr-2 text-xs border-vista-secondary/30" onClick={() => setSelectedTags([])}>
                          Очистить
                        </Button>
                        <Button size="sm" className="text-xs bg-vista-primary text-vista-dark hover:bg-vista-primary/90" onClick={() => setTagsPopoverOpen(false)}>
                          Готово
                        </Button>
                      </div>
                    </div>
            ...
            {hasActiveFilters && (<div className="flex flex-wrap items-center gap-2 mt-2">
                <span className="text-sm text-vista-light/70">Активные фильтры:</span>
                
                {searchQuery && (<Badge variant="secondary" className="bg-vista-secondary/20 text-vista-light">
                    Поиск: {searchQuery}
                ...
                {selectedAuthor && (<Badge variant="secondary" className="bg-vista-secondary/20 text-vista-light">
                    Автор: {((_a = usersData.find((u) => u.id === selectedAuthor)) === null || _a === void 0 ? void 0 : _a.name) || 'Неизвестно'}
                    <button onClick={() => setSelectedAuthor(null)} className="ml-1">
                      <X className="h-3 w-3"/>
                    </button>
                  </Badge>)}
                
                {selectedCategory && (<Badge variant="secondary" className="bg-vista-secondary/20 text-vista-light">
                    Категория: {(_b = categoriesData.find((c) => c.id === selectedCategory)) === null || _b === void 0 ? void 0 : _b.name}
                    <button onClick={() => setSelectedCategory(null)} className="ml-1">
                      <X className="h-3 w-3"/>
                    </button>
                  </Badge>)}
                
                {selectedTagNames.map((tagName) => (<Badge key={tagName} variant="secondary" className="bg-vista-secondary/20 text-vista-light">
                    Тег: {tagName}
                    <button onClick={() => {
                    var _a;
                    const tagId = (_a = tagsData.find((t) => t.name === tagName)) === null || _a === void 0 ? void 0 : _a.id;
                    if (tagId) {
                ...
                <Button variant="outline" size="sm" onClick={resetFilters} className="ml-auto text-vista-light/70 hover:text-vista-light border-vista-secondary/30">
                  Сбросить все
                </Button>
              </div>)}
          </div>
          ...
          {isLoading ? (<div className="flex justify-center py-8">
            ...
            </div>) : filteredExercises.length > 0 ? (<div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredExercises.map((exercise) => {
                var _a, _b;
                return (<div key={exercise.id} className="rounded-md border border-vista-secondary/30 bg-vista-dark/70 hover:bg-vista-dark/90 transition overflow-hidden flex flex-col cursor-pointer hover:ring-1 hover:ring-vista-primary hover:ring-offset-0 hover:ring-offset-gray-800/20 shadow-md hover:shadow-xl" onClick={() => {
                        setPreviewExercise(exercise);
                        setIsPreviewOpen(true);
                    }}>
                    {/* Медиа файл с оптимизацией загрузки */}
                    <div className="h-48 relative bg-vista-secondary/10">
                      {exercise.mediaItems && exercise.mediaItems.length > 0 ? (exercise.mediaItems[0].type === 'IMAGE' ? (<img src={exercise.mediaItems[0].publicUrl} alt={exercise.title} className="w-full h-full object-cover" loading="lazy" decoding="async" width={300} height={200} onError={(e) => {
                            var _a, _b;
                            // При ошибке загрузки показываем заглушку
                            const target = e.target;
                            target.style.display = 'none';
                            (_a = target.parentElement) === null || _a === void 0 ? void 0 : _a.classList.add('flex', 'items-center', 'justify-center');
                            ...
                            (_b = target.parentElement) === null || _b === void 0 ? void 0 : _b.appendChild(fallback);
                        }}/>) : exercise.mediaItems[0].type === 'VIDEO' ? (<div className="relative w-full h-full">
                            <div className="w-full h-full bg-vista-dark/20">
                              <video src={exercise.mediaItems[0].publicUrl} className="w-full h-full object-cover" preload="none" muted playsInline poster={exercise.mediaItems[0].publicUrl + '?poster=true'}/>
                            </div>
                          ...
                          </div>) : (<div className="w-full h-full flex items-center justify-center text-vista-light/30">
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      ...
                      <div className="mt-2">
                        <span className="inline-block px-2 py-1 text-xs rounded-md bg-vista-primary/20 text-vista-primary">
                          {((_a = exercise.category) === null || _a === void 0 ? void 0 : _a.name) || 'Без категории'}
                        </span>
                      </div>
                      
                      <div className="mt-2 text-xs text-vista-light/70">
                        Автор: {((_b = exercise.author) === null || _b === void 0 ? void 0 : _b.name) || 'Неизвестно'}
                      </div>
                      
                      <div className="mt-2 flex flex-wrap gap-1">
                        {exercise.tags.slice(0, 3).map((tag) => (<span key={tag.id} className="px-2 py-1 text-xs rounded-md bg-vista-secondary/20 text-vista-light/80">
                            {tag.name}
                          </span>))}
                        {exercise.tags.length > 3 && (<span className="px-2 py-1 text-xs rounded-md bg-vista-secondary/20 text-vista-light/80">
              ...
              {pagination.totalPages > 1 && (<div className="flex justify-center items-center space-x-2 mt-6">
                  <Button variant="outline" size="sm" onClick={goToPrevPage} disabled={!pagination.hasPreviousPage} className="border-vista-secondary/30 text-vista-light">
                    Назад
                  </Button>
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    return [...new Set(pages)].map((page) => {
                        if (page === 'ellipsis1' || page === 'ellipsis2') {
                            return (<span key={page} className="w-9 h-9 flex items-center justify-center text-vista-light/60">
                              ...
                            </span>);
                        }
                        return (<Button key={page} variant={currentPage === page ? "default" : "outline"} size="sm" onClick={() => goToPage(page)} className={cn("w-9 h-9 p-0", currentPage === page
                  ...
                  <Button variant="outline" size="sm" onClick={goToNextPage} disabled={!pagination.hasNextPage} className="border-vista-secondary/30 text-vista-light">
                    Вперед
                  </Button>
                </div>)}
            </div>) : (<div className="text-center py-8 border border-dashed border-vista-secondary/30 rounded-md">
          ...
          {previewExercise && (<>
            ...
            <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="edit-title" name="title" value={editExerciseForm.title} onChange={handleEditInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light"/>
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea id="edit-description" name="description" value={editExerciseForm.description} onChange={handleEditInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light min-h-[120px]"/>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input id="edit-width" name="width" type="number" value={editExerciseForm.width} onChange={handleEditInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light"/>
                      </div>
                      
                      <div className="space-y-2">
                        <Label htmlFor="edit-length" className="text-vista-light">Длина (м)</Label>
                        <Input id="edit-length" name="length" type="number" value={editExerciseForm.length} onChange={handleEditInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light"/>
                      </div>
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="edit-category" className="text-vista-light">Категория <span className="text-vista-primary/80">*</span></Label>
                      <Select value={editExerciseForm.categoryId} onValueChange={(value) => {
                    handleEditCategoryChange(value);
                    // Подсказка пользователю с выбором тегов
                    if (value && tagsData.filter((tag) => tag.exerciseCategoryId === value).length > 0) {
                        setTimeout(() => {
                        ...
                        <SelectTrigger id="edit-category" className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light">
                          <SelectValue placeholder="Выберите категорию"/>
                        </SelectTrigger>
                      ...
                      {editExerciseForm.categoryId && (<p className="text-xs text-vista-light/60">
                    ...
                    <div className="space-y-2">
                      ...
                      {editExerciseForm.categoryId ? (filteredEditTags.length > 0 ? (<>
                        ...
                        return (<div key={tag.id} className={`flex items-center space-x-2 p-2 rounded-md cursor-pointer hover:bg-vista-secondary/20 ${isSelected ? 'bg-vista-primary/20' : ''}`} onClick={() => {
                                setEditExerciseForm((prev) => (Object.assign(Object.assign({}, prev), { tags: isSelected
                                        ? prev.tags.filter(id => id !== tag.id)
                                        : [...prev.tags, tag.id] })));
                            }}>
                                      <div className={`flex h-4 w-4 items-center justify-center rounded-sm border ${isSelected
                                ? 'border-vista-primary bg-vista-primary'
                                : 'border-vista-secondary/50'}`}>
                                        {isSelected && <Check className="h-3 w-3 text-vista-dark"/>}
                                      </div>
                            ...
                            {editExerciseForm.tags.length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
                                {editExerciseForm.tags.map(tagId => {
                            const tag = tagsData.find((t) => t.id === tagId);
                            return tag ? (<Badge key={tag.id} className="bg-vista-primary/20 text-vista-primary">
                                      {tag.name}
                                      <button type="button" onClick={(e) => {
                                    e.stopPropagation();
                                    setEditExerciseForm(prev => (Object.assign(Object.assign({}, prev), { tags: prev.tags.filter(id => id !== tag.id) })));
                                ...
                                {editExerciseForm.tags.length > 1 && (<button type="button" onClick={() => setEditExerciseForm(prev => (Object.assign(Object.assign({}, prev), { tags: [] })))} className="text-xs text-vista-light/70 hover:text-vista-light">
                                    Очистить все
                                  </button>)}
                              </div>)}
                          ...
                          </div>)) : (<div className="py-2 px-2 border border-vista-secondary/30 rounded-md text-center text-vista-light/60">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col items-center border-2 border-dashed border-vista-secondary/30 rounded-md p-4 text-center">
                        {filePreviewEdit ? (<div className="relative w-full">
                            <img src={filePreviewEdit} alt="Preview" className="max-h-[200px] mx-auto object-contain rounded-md"/>
                            <button type="button" onClick={() => {
                        setFilePreviewEdit(null);
                        setEditExerciseForm(Object.assign(Object.assign({}, editExerciseForm), { file: null }));
                        ...
                        <Input id="edit-file" type="file" onChange={handleEditFileChange} className={`mt-2 ${filePreviewEdit ? 'hidden' : ''}`}/>
                      </div>
                    </div>
                  </div>) : (
            // Просмотр упражнения
            <>
                    ...
                    {previewExercise.mediaItems && previewExercise.mediaItems.length > 0 && (<div className="relative bg-vista-secondary/10 rounded-md overflow-hidden" style={{ maxHeight: '400px' }}>
                        {previewExercise.mediaItems[0].type === 'IMAGE' ? (<img src={previewExercise.mediaItems[0].publicUrl} alt={previewExercise.title} className="w-full h-full object-contain max-h-[400px] mx-auto"/>) : previewExercise.mediaItems[0].type === 'VIDEO' ? (<video src={previewExercise.mediaItems[0].publicUrl} className="w-full h-full object-contain max-h-[400px] mx-auto" controls/>) : null}
                      </div>)}
                    
                    {/* Информация об упражнении */}
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span className="px-3 py-1 rounded-full bg-vista-primary/20 text-vista-primary text-sm">
                        {((_c = previewExercise.category) === null || _c === void 0 ? void 0 : _c.name) || 'Без категории'}
                      </span>
                      
                      {previewExercise.tags.map((tag) => (<span key={tag.id} className="px-3 py-1 rounded-full bg-vista-secondary/20 text-vista-light/80 text-sm">
                          {tag.name}
                        </span>))}
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {(previewExercise.width || previewExercise.length) && (<div className="flex gap-4">
                          {previewExercise.width && (<div>
                      ...
                      <div>
                        ...
                        <p className="text-vista-light">{((_d = previewExercise.author) === null || _d === void 0 ? void 0 : _d.name) || 'Неизвестно'}</p>
                      </div>
                    </div>
                  </>)}
                ...
                <div className="flex justify-end gap-2 mt-4">
                  {!isEditMode ? (<>
                      <Button variant="outline" onClick={startEditExercise} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
                        Редактировать
                      </Button>
                      <Button variant="destructive" onClick={deleteExercise} className="bg-red-500/20 text-red-400 hover:bg-red-500/30 hover:text-red-300">
                        Удалить
                      </Button>
                    </>) : (<>
                      <Button variant="outline" onClick={() => setIsEditMode(false)} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
                        Отмена
                      </Button>
                      <Button onClick={saveEditedExercise} className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
                        Сохранить
                      </Button>
                    </>)}
                </div>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto bg-vista-dark/95 border-vista-secondary/30 text-vista-light backdrop-blur-xl">
          ...
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="new-title" name="title" value={newExercise.title} onChange={(e) => {
            setNewExercise(Object.assign(Object.assign({}, newExercise), { title: e.target.value }));
            ...
            <div className="space-y-2">
              ...
              <Textarea id="new-description" name="description" value={newExercise.description} onChange={(e) => {
            setNewExercise(Object.assign(Object.assign({}, newExercise), { description: e.target.value }));
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="new-width" name="width" type="number" value={newExercise.width} onChange={(e) => {
            setNewExercise(Object.assign(Object.assign({}, newExercise), { width: e.target.value }));
        }} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light"/>
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="new-length" className="text-vista-light">Длина (м)</Label>
                <Input id="new-length" name="length" type="number" value={newExercise.length} onChange={(e) => {
            setNewExercise(Object.assign(Object.assign({}, newExercise), { length: e.target.value }));
        }} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light"/>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={newExercise.categoryId} onValueChange={(value) => {
            setNewExercise(Object.assign(Object.assign({}, newExercise), { categoryId: value, tags: [] // Сбрасываем теги при смене категории
             }));
            // Подсказка пользователю с выбором тегов
            if (value && tagsData.filter((tag) => tag.exerciseCategoryId === value).length > 0) {
                setTimeout(() => {
                ...
                <SelectTrigger id="new-category" className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light">
                  <SelectValue placeholder="Выберите категорию"/>
                </SelectTrigger>
                <SelectContent className="bg-vista-dark border-vista-secondary/30 text-vista-light">
                  {categoriesData.map((category) => (<SelectItem key={category.id} value={category.id}>
              ...
              {newExercise.categoryId && (<p className="text-xs text-vista-light/60">
                  Теперь вы можете выбрать теги для этой категории.
                </p>)}
            </div>
            ...
            <div className="space-y-2">
              ...
              {newExercise.categoryId ? (filteredTags.length > 0 ? (<>
                    <div className="border border-vista-secondary/30 rounded-md">
                      <div style={{ maxHeight: '180px', overflowY: 'auto' }} className="space-y-1 p-1">
                        {filteredTags.map((tag) => {
                const isSelected = newExercise.tags.includes(tag.id);
                return (<div key={tag.id} className={`flex items-center space-x-2 p-2 rounded-md cursor-pointer hover:bg-vista-secondary/20 ${isSelected ? 'bg-vista-primary/20' : ''}`} onClick={() => {
                        setNewExercise((prev) => (Object.assign(Object.assign({}, prev), { tags: isSelected
                                ? prev.tags.filter(id => id !== tag.id)
                                : [...prev.tags, tag.id] })));
                    }}>
                              <div className={`flex h-4 w-4 items-center justify-center rounded-sm border ${isSelected
                    ...
                    {newExercise.tags.length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
                    ...
                    return tag ? (<Badge key={tag.id} className="bg-vista-primary/20 text-vista-primary">
                              ...
                              <button type="button" onClick={(e) => {
                            e.stopPropagation();
                            setNewExercise(prev => (Object.assign(Object.assign({}, prev), { tags: prev.tags.filter(id => id !== tag.id) })));
                        }} className="ml-1 hover:text-vista-light">
                                <X className="h-3 w-3"/>
                              </button>
                            </Badge>) : null;
                        ...
                        {newExercise.tags.length > 1 && (<button type="button" onClick={() => setNewExercise(prev => (Object.assign(Object.assign({}, prev), { tags: [] })))} className="text-xs text-vista-light/70 hover:text-vista-light">
                            Очистить все
                          </button>)}
                      </div>)}
                  </>) : (<div className="py-2 px-2 border border-vista-secondary/30 rounded-md text-center text-vista-light/60">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col items-center border-2 border-dashed border-vista-secondary/30 rounded-md p-4 text-center">
                {filePreview ? (<div className="relative w-full">
                    <img src={filePreview} alt="Preview" className="max-h-[200px] mx-auto object-contain rounded-md"/>
                    <button type="button" onClick={() => {
                setFilePreview(null);
                setNewExercise(Object.assign(Object.assign({}, newExercise), { file: null }));
            }} className="absolute top-2 right-2 rounded-full bg-vista-dark/70 p-1">
                      <X className="h-4 w-4 text-vista-light"/>
                    </button>
                  </div>) : (<>
                ...
                <Input id="new-file" type="file" onChange={(e) => {
            const file = e.target.files ? e.target.files[0] : null;
            if (file) {
                setNewExercise(Object.assign(Object.assign({}, newExercise), { file }));
                // Предпросмотр файла
                const reader = new FileReader();
                reader.onloadend = () => {
