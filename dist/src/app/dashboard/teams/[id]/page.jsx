...
export default function TeamPage() {
    ...
    const [selectedCoachIdsToDelete, setSelectedCoachIdsToDelete] = useState([]);
    ...
    useEffect(() => {
        ...
        if ((session === null || session === void 0 ? void 0 : session.user) && teamId) {
            fetchTeamData();
            fetchPlayersData();
            fetchCoachesData();
        }
    ...
    useEffect(() => {
        ...
        const listeners = [];
        // @ts-ignore
        if (window && window.__vite_plugin_react_preamble_installed)
            return; // для Vite/Next.js dev mode
    ...
    const handleInputChange = (e) => {
        ...
        setNewPlayer(prev => (Object.assign(Object.assign({}, prev), { [name]: value })));
    };
    ...
    const handleUpdatePlayerStatus = async (status) => {
        if (!selectedPlayer)
            return;
        try {
            ...
            // Обновляем список игроков
            setPlayers(prev => prev.map(player => player.id === selectedPlayer.id ? Object.assign(Object.assign({}, player), { status }) : player));
            // Закрываем модальное окно
    ...
    const handleOpenStatusPlayersDialog = (status, title) => {
    ...
    const handleCoachSelectionChange = (coachId, isSelected) => {
        if (isSelected) {
    ...
    const handleCoachSelectionToDeleteChange = (userId, isSelected) => {
        if (isSelected) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Button variant="outline" size="sm" onClick={handleBackToList} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
          ...
          <div className="bg-vista-dark/70 border border-vista-secondary/30 py-1 px-3 rounded-md text-vista-light h-9 flex items-center">
            <div>
              <h1 className="text-sm font-medium">
                {isLoading ? 'Загрузка...' : (team === null || team === void 0 ? void 0 : team.name) || 'Команда'}
              </h1>
              {(team === null || team === void 0 ? void 0 : team.description) && (<p className="text-xs text-vista-light/70">{team.description}</p>)}
            </div>
          </div>
        </div>
        
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark border-none" onClick={activeTab === 'squad' ? handleOpenAddPlayerDialog : handleOpenAddCoachDialog}>
            <PlusIcon className="w-4 h-4 mr-2"/>
            {activeTab === 'squad' ? 'Добавить игрока' : 'Добавить тренера'}
          </Button>
          <Button variant="outline" size="sm" className="bg-red-500/80 hover:bg-red-500 text-white border-none" onClick={activeTab === 'squad' ? handleOpenDeletePlayersDialog : handleOpenDeleteCoachesDialog} disabled={activeTab === 'squad' ? players.length === 0 : coaches.length === 0}>
      ...
      {isLoading ? (<div className="flex justify-center py-12">
        ...
        </div>) : error ? (<Card className="bg-vista-dark/50 border-vista-secondary/30">
          <CardContent className="p-6">
            <div className="text-center py-8">
              ...
              <Button className="mt-4 bg-vista-primary hover:bg-vista-primary/90 text-vista-dark" onClick={handleBackToList}>
                Вернуться к списку команд
              </Button>
            </div>
          </CardContent>
        </Card>) : team ? (<div className="space-y-6">
          {/* Табы для разделов */}
          <Tabs defaultValue="squad" className="w-full" onValueChange={(value) => setActiveTab(value)}>
            <div className="flex items-center justify-between bg-vista-dark/50 border-b border-vista-secondary/30">
              ...
              {activeTab === 'squad' && (<div className="flex items-center space-x-2 px-2">
                  <div className="flex items-center rounded-md bg-green-500/20 px-2 py-1 cursor-pointer hover:bg-green-500/30" onClick={() => handleOpenStatusPlayersDialog('ready', 'Готов')}>
                  ...
                  <div className="flex items-center rounded-md bg-blue-500/20 px-2 py-1 cursor-pointer hover:bg-blue-500/30" onClick={() => handleOpenStatusPlayersDialog('rehabilitation', 'Реабилитация')}>
                  ...
                  <div className="flex items-center rounded-md bg-yellow-500/20 px-2 py-1 cursor-pointer hover:bg-yellow-500/30" onClick={() => handleOpenStatusPlayersDialog('sick', 'Болеет')}>
                  ...
                  <div className="flex items-center rounded-md bg-purple-500/20 px-2 py-1 cursor-pointer hover:bg-purple-500/30" onClick={() => handleOpenStatusPlayersDialog('study', 'Учеба')}>
                  ...
                  <div className="flex items-center rounded-md bg-gray-500/20 px-2 py-1 cursor-pointer hover:bg-gray-500/30" onClick={() => handleOpenStatusPlayersDialog('other', 'Другое')}>
            ...
            <TabsContent value="squad">
              <Card className="bg-vista-dark/50 border-vista-secondary/30">
                ...
                <CardContent>
                  {isLoadingPlayers ? (<div className="flex justify-center py-8">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-vista-primary"></div>
                    </div>) : players.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8 gap-3">
                      {players.map(player => (<div key={player.id} className="bg-gray-50/30 rounded-md overflow-hidden cursor-pointer group relative transition-all duration-200 hover:ring-1 hover:ring-vista-primary hover:ring-offset-0 hover:ring-offset-gray-800/20 shadow-md" onClick={() => router.push(`/dashboard/teams/${teamId}/players/${player.id}`)}>
                          {/* Номер игрока в верхнем правом углу */}
                          {player.number && (<div className="absolute top-[2px] right-[2px] bg-gray-800/85 text-vista-primary rounded-md px-1 py-0.5 text-sm font-medium z-50 shadow-md">
                              {player.number}
                            </div>)}
                          ...
                          <div className="w-full aspect-square relative overflow-hidden">
                            ...
                            {player.imageUrl ? (<img src={player.imageUrl} alt={`${player.firstName} ${player.lastName}`} className="w-full h-full object-cover z-10 relative" style={{ background: 'transparent' }}/>) : (<div className="w-full h-full flex items-center justify-center z-10 relative">
                          ...
                          <div className="p-2 bg-gray-800/90">
                            <div className="text-center">
                              {player.firstName && (<p className="text-vista-light/70 text-xs">{player.firstName}</p>)}
                              {player.lastName && (<p className="text-vista-light font-medium">{player.lastName}</p>)}
                              <div className="flex justify-between items-center mt-1">
                                {player.position && (<p className="text-[10px] text-vista-light/70 uppercase truncate max-w-[40%]">{player.position}</p>)}
                                <div className={`text-[10px] px-1.5 py-0.5 rounded-sm cursor-pointer truncate max-w-[55%] text-center ml-auto ${player.status === 'rehabilitation' ? 'bg-blue-500/20 text-blue-300' :
                        player.status === 'sick' ? 'bg-yellow-500/20 text-yellow-300' :
                            player.status === 'study' ? 'bg-purple-500/20 text-purple-300' :
                                player.status === 'other' ? 'bg-gray-500/20 text-gray-300' :
                                    'bg-green-500/20 text-green-300'}`} onClick={(e) => {
                        e.stopPropagation();
                      ...
                      <Button onClick={handleOpenAddPlayerDialog} className="mt-4 bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
                        <PlusIcon className="w-4 h-4 mr-2"/>
            ...
            <TabsContent value="staff">
              <Card className="bg-vista-dark/50 border-vista-secondary/30">
                ...
                <CardContent>
                  {isLoadingCoaches ? (<div className="flex justify-center py-8">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-vista-primary"></div>
                    </div>) : coaches.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {coaches.map(coach => (<div key={coach.id} className="bg-vista-dark/70 rounded-md overflow-hidden shadow-md border border-vista-secondary/30">
                          <div className="p-4 flex flex-col items-center text-center">
                            <div className="w-20 h-20 rounded-full overflow-hidden relative bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)] mb-3">
                              {coach.user.imageUrl ? (<img src={coach.user.imageUrl} alt={coach.user.name || 'Тренер'} className="w-full h-full object-cover"/>) : (<div className="w-full h-full flex items-center justify-center">
                            ...
                            {coach.role && (<div className="mt-2 px-2 py-1 bg-vista-primary/20 rounded text-xs text-vista-primary">
                                {coach.role}
                              </div>)}
                          </div>
                        </div>))}
                      ...
                      <Button onClick={handleOpenAddCoachDialog} className="mt-4 bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
                        <PlusIcon className="w-4 h-4 mr-2"/>
      ...
      <Dialog open={isAddPlayerDialogOpen} onOpenChange={setIsAddPlayerDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-md">
          ...
          <form onSubmit={handleSubmitPlayer}>
            {formError && (<div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-500">
                {formError}
              </div>)}
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="firstName" name="firstName" value={newPlayer.firstName} onChange={handleInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light" placeholder="Введите имя игрока" disabled={isSubmitting}/>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="lastName" name="lastName" value={newPlayer.lastName} onChange={handleInputChange} className="bg-vista-dark/70 border-vista-secondary/30 text-vista-light" placeholder="Введите фамилию игрока" disabled={isSubmitting}/>
              </div>
            </div>
            
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setIsAddPlayerDialogOpen(false)} disabled={isSubmitting} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
                Отмена
              </Button>
              <Button type="submit" disabled={isSubmitting} className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
                {isSubmitting ? (<>
      ...
      <Dialog open={isDeletePlayersDialogOpen} onOpenChange={setIsDeletePlayersDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-md">
          ...
          {deleteError && (<div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-500">
              {deleteError}
            </div>)}

          <div className="py-4 max-h-[300px] overflow-y-auto">
            {players.length > 0 ? (<div className="space-y-2">
                {players.map(player => (<div key={player.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-vista-dark/70">
                    <Checkbox id={`player-${player.id}`} checked={selectedPlayerIds.includes(player.id)} onCheckedChange={(checked) => handlePlayerSelectionChange(player.id, checked === true)} className="border-vista-secondary/50"/>
                    <Label htmlFor={`player-${player.id}`} className="cursor-pointer flex-1 flex items-center">
                      <div className="flex items-center w-full">
                        {player.imageUrl ? (<div className="w-10 h-10 rounded-full overflow-hidden mr-3 relative bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                            <img src={player.imageUrl} alt={`${player.firstName} ${player.lastName}`} className="w-full h-full object-cover"/>
                          </div>) : (<div className="w-10 h-10 rounded-full mr-3 flex items-center justify-center bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                        ...
                        {player.number && (<div className="ml-auto bg-vista-primary/20 rounded-full w-6 h-6 flex items-center justify-center">
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsDeletePlayersDialogOpen(false)} disabled={isDeleting} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
              Отмена
            </Button>
            <Button type="button" onClick={handleDeletePlayers} disabled={isDeleting || selectedPlayerIds.length === 0} className="bg-red-500/80 hover:bg-red-500 text-white">
              {isDeleting ? (<>
      ...
      <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-xs">
          ...
          {statusError && (<div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-500">
              {statusError}
            </div>)}

          <div className="py-4">
            <div className="space-y-2">
              <div className="flex items-center p-2 rounded-md bg-green-500/20 hover:bg-green-500/30 cursor-pointer" onClick={() => handleUpdatePlayerStatus('ready')}>
                <div className="w-3 h-3 bg-green-500 rounded-full mr-3"></div>
                <span className="text-green-300">Готов</span>
              </div>
              
              <div className="flex items-center p-2 rounded-md bg-blue-500/20 hover:bg-blue-500/30 cursor-pointer" onClick={() => handleUpdatePlayerStatus('rehabilitation')}>
                <div className="w-3 h-3 bg-blue-500 rounded-full mr-3"></div>
                <span className="text-blue-300">Реабилитация</span>
              </div>
              
              <div className="flex items-center p-2 rounded-md bg-yellow-500/20 hover:bg-yellow-500/30 cursor-pointer" onClick={() => handleUpdatePlayerStatus('sick')}>
                <div className="w-3 h-3 bg-yellow-500 rounded-full mr-3"></div>
                <span className="text-yellow-300">Болеет</span>
              </div>
              
              <div className="flex items-center p-2 rounded-md bg-purple-500/20 hover:bg-purple-500/30 cursor-pointer" onClick={() => handleUpdatePlayerStatus('study')}>
                <div className="w-3 h-3 bg-purple-500 rounded-full mr-3"></div>
                <span className="text-purple-300">Учеба</span>
              </div>
              
              <div className="flex items-center p-2 rounded-md bg-gray-500/20 hover:bg-gray-500/30 cursor-pointer" onClick={() => handleUpdatePlayerStatus('other')}>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsStatusDialogOpen(false)} disabled={isUpdatingStatus} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
              Отмена
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isStatusPlayersDialogOpen} onOpenChange={setIsStatusPlayersDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-md">
          ...
          <div className="py-4 max-h-[350px] overflow-y-auto">
            {players.filter(player => selectedStatus === 'ready'
            ? player.status === 'ready' || !player.status
            : player.status === selectedStatus).length > 0 ? (<div className="space-y-2">
                {players
                .filter(player => selectedStatus === 'ready'
                ? player.status === 'ready' || !player.status
                : player.status === selectedStatus)
                .map(player => (<div key={player.id} className="flex items-center p-2 rounded-md hover:bg-vista-dark/70 cursor-pointer" onClick={() => router.push(`/dashboard/teams/${teamId}/players/${player.id}`)}>
                      <div className="flex items-center w-full">
                        {player.imageUrl ? (<div className="w-10 h-10 rounded-full overflow-hidden mr-3 relative bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                            <img src={player.imageUrl} alt={`${player.firstName} ${player.lastName}`} className="w-full h-full object-cover"/>
                          </div>) : (<div className="w-10 h-10 rounded-full mr-3 flex items-center justify-center bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                        ...
                        {player.number && (<div className="ml-auto bg-vista-primary/20 rounded-full w-7 h-7 flex items-center justify-center">
                            <span className="text-xs font-medium text-vista-primary">{player.number}</span>
                          </div>)}
                      </div>
                    </div>))}
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsStatusPlayersDialogOpen(false)} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
              Закрыть
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isAddCoachDialogOpen} onOpenChange={setIsAddCoachDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-md">
          ...
          {addCoachError && (<div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-500">
              {addCoachError}
            </div>)}

          <div className="py-4 max-h-[300px] overflow-y-auto">
            {availableCoaches.length > 0 ? (<div className="space-y-2">
                {availableCoaches.map(coach => (<div key={coach.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-vista-dark/70">
                    <Checkbox id={`coach-${coach.id}`} checked={selectedCoachIds.includes(coach.id)} onCheckedChange={(checked) => handleCoachSelectionChange(coach.id, checked === true)} className="border-vista-secondary/50"/>
                    <Label htmlFor={`coach-${coach.id}`} className="cursor-pointer flex-1 flex items-center">
                      <div className="flex items-center w-full">
                        <div className="w-10 h-10 rounded-full overflow-hidden mr-3 relative bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                          {coach.imageUrl ? (<img src={coach.imageUrl} alt={coach.name || 'Тренер'} className="w-full h-full object-cover"/>) : (<div className="w-10 h-10 rounded-full mr-3 flex items-center justify-center bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                              <UserIcon className="w-5 h-5 text-slate-300"/>
                            </div>)}
                        </div>
                        <div className="flex-1">
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsAddCoachDialogOpen(false)} disabled={isAddingCoaches} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
              Отмена
            </Button>
            <Button type="button" onClick={handleAddCoaches} disabled={isAddingCoaches || selectedCoachIds.length === 0} className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark">
              {isAddingCoaches ? (<>
                  <div className="animate-spin mr-2 h-4 w-4 border-2 border-vista-dark border-t-transparent rounded-full"></div>
                  Добавление...
      ...
      <Dialog open={isDeleteCoachesDialogOpen} onOpenChange={setIsDeleteCoachesDialogOpen}>
        <DialogContent className="bg-vista-dark/95 border-vista-secondary/30 text-vista-light max-w-md">
          ...
          {deleteCoachError && (<div className="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-md text-red-500">
              {deleteCoachError}
            </div>)}

          <div className="py-4 max-h-[300px] overflow-y-auto">
            {coaches.length > 0 ? (<div className="space-y-2">
                {coaches.map(coach => (<div key={coach.id} className="flex items-center space-x-3 p-2 rounded-md hover:bg-vista-dark/70">
                    <Checkbox id={`coach-delete-${coach.userId}`} checked={selectedCoachIdsToDelete.includes(coach.userId)} onCheckedChange={(checked) => handleCoachSelectionToDeleteChange(coach.userId, checked === true)} className="border-vista-secondary/50"/>
                    <Label htmlFor={`coach-delete-${coach.userId}`} className="cursor-pointer flex-1 flex items-center">
                      <div className="flex items-center w-full">
                        <div className="w-10 h-10 rounded-full overflow-hidden mr-3 relative bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                          {coach.user.imageUrl ? (<img src={coach.user.imageUrl} alt={coach.user.name || 'Тренер'} className="w-full h-full object-cover"/>) : (<div className="w-10 h-10 rounded-full mr-3 flex items-center justify-center bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)]">
                              <UserIcon className="w-5 h-5 text-slate-300"/>
                            </div>)}
                        </div>
                        <div className="flex-1">
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsDeleteCoachesDialogOpen(false)} disabled={isDeletingCoaches} className="border-vista-secondary/30 text-vista-light hover:bg-vista-secondary/20">
              Отмена
            </Button>
            <Button type="button" onClick={handleDeleteCoaches} disabled={isDeletingCoaches || selectedCoachIdsToDelete.length === 0} className="bg-red-500/80 hover:bg-red-500 text-white">
              {isDeletingCoaches ? (<>
                  <div className="animate-spin mr-2 h-4 w-4 border-2 border-white border-t-transparent rounded-full"></div>
                  Удаление...
