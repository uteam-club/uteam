...
import { FileTextIcon, FileX2Icon, UserIcon, DownloadIcon, SearchIcon } from 'lucide-react';
...
export default function DocumentsPage() {
    ...
    useEffect(() => {
        if (searchQuery.trim() === '') {
            ...
            const filtered = allPlayers.filter(player => `${player.firstName} ${player.lastName}`.toLowerCase().includes(query) ||
    ...
    const downloadExcel = () => {
        ...
        try {
            ...
            const teamName = selectedTeamId === 'all'
                ? 'Все команды'
                : ((_a = teams.find(team => team.id === selectedTeamId)) === null || _a === void 0 ? void 0 : _a.name) || 'Игроки';
    ...
    const renderDocumentIcon = (document) => {
        if (!document) {
            return (<div className="flex items-center text-vista-light/50">
          <FileX2Icon className="w-5 h-5 mr-1"/>
        </div>);
        }
        return (<a href={document.publicUrl} target="_blank" rel="noopener noreferrer" className="flex items-center text-vista-primary hover:text-vista-primary/80 transition-colors">
        ...
        <CardContent>
          <div className="flex gap-4 items-end">
            ...
            <div className="w-64 relative">
              <Input type="text" placeholder="Поиск игрока по имени" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="bg-vista-dark/70 border-vista-secondary/50 text-vista-light pr-9 focus:border-vista-primary focus:ring-1 focus:ring-vista-primary/50"/>
              <SearchIcon className="absolute right-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-vista-light/50"/>
            </div>
            
            <Button variant="outline" size="icon" onClick={downloadExcel} className="border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20 shadow-sm" title="Скачать список игроков в Excel">
            ...
            <Table>
              ...
              <TableBody>
                {players.length === 0 ? (<TableRow>
                  ...
                  </TableRow>) : (players.map((player) => (<TableRow key={player.id} className="hover:bg-vista-dark/70 border-b border-vista-secondary/50 shadow-md">
                      <TableCell className="sticky left-0 bg-vista-dark/70 py-1.5 w-[250px] min-w-[250px] shadow-sm">
                        <div className="flex items-center space-x-2">
                          <div className="h-9 w-9 rounded-full overflow-hidden bg-gradient-to-t from-[rgba(52,64,84,0.5)] to-[rgba(230,247,255,0.65)] flex items-center justify-center shadow-sm">
                            {player.imageUrl ? (<img src={player.imageUrl} alt={`${player.firstName} ${player.lastName}`} className="h-full w-full object-cover" onError={(e) => {
                    e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(player.firstName[0] + player.lastName[0])}&background=162a5b&color=fff&size=100`;
                }}/>) : (<UserIcon className="h-5 w-5 text-slate-300"/>)}
                          </div>
                          <div>
                            <Link href={`/dashboard/teams/${player.team.id}/players/${player.id}`} className="font-medium text-vista-light/90 hover:text-vista-primary transition-colors">
