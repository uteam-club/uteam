...
import { format, addMonths, subMonths, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, getDay, addDays, isToday, isSameDay, parseISO, startOfWeek, endOfWeek, addWeeks, subWeeks } from 'date-fns';
import { ChevronLeft, ChevronRight, Users, Clock, Trophy, CheckCircle2, AlertCircle, CalendarClock, Medal, Handshake } from 'lucide-react';
...
export default function CalendarPage() {
    ...
    const generateCalendarDays = () => {
        ...
        const prevDays = Array.from({ length: startDayOfWeek }, (_, i) => addDays(monthStart, -(startDayOfWeek - i)));
        ...
        const nextDays = Array.from({ length: daysToAdd }, (_, i) => addDays(monthEnd, i + 1));
    ...
    const fetchTrainings = async () => {
        ...
        try {
            ...
            await Promise.all(selectedTeams.map(async (teamId) => {
                var _a;
                const teamName = ((_a = teams.find(team => team.id === teamId)) === null || _a === void 0 ? void 0 : _a.name) || '';
                try {
                    ...
                    const processedTrainings = await Promise.all(trainingsData.map(async (training) => {
                            ...
                            try {
                                ...
                                if (attendanceResponse.ok) {
                                    ...
                                    if (attendanceData && attendanceData.length > 0) {
                                        const hasAttendance = attendanceData.some((player) => player.attendance && player.attendance.status === 'TRAINED');
                                        if (hasAttendance) {
                                            status = 'completed';
                                        }
                                    }
                        ...
                        return Object.assign(Object.assign({}, training), { teamName,
                            type,
                            status });
                    }));
                    // Обработка матчей
        ...
        try {
            ...
            setTrainings(prevTrainings => prevTrainings.map(training => {
                if (training.id === trainingId) {
                    ...
                    return Object.assign(Object.assign({}, training), { status: updatedTraining.status });
                }
                return training;
            }));
            toast({
    ...
    const navigateToEvent = (training) => {
    ...
    const getTrainingTypeDisplay = (training) => {
        if (training.type === 'MATCH') {
            return 'Матч';
        }
    ...
    const getMatchTypeIcon = (training) => {
        if (training.type !== 'MATCH')
            return null;
    ...
    const getTrainingTypeIcon = (training) => {
        if (training.type === 'MATCH') {
    ...
    const getTrainingStyles = (training) => {
        if (training.type === 'MATCH') {
    ...
    const getMatchResultClass = (training) => {
        ...
        <CardHeader className="p-2 md:p-3 bg-vista-dark/50 border-b border-vista-secondary/40 flex flex-row justify-between items-center">
          ...
          <Popover open={isTeamSelectOpen} onOpenChange={setIsTeamSelectOpen}>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="w-auto justify-start border-vista-secondary/50 text-vista-light shadow-sm h-8 py-1">
                <Users className="mr-2 h-3.5 w-3.5"/>
                <span className="text-sm">{selectedTeamsText()}</span>
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-full min-w-[240px] p-0 bg-vista-dark border-vista-secondary/50 shadow-xl" align="start" sideOffset={8}>
              <div className="p-4 space-y-4">
                <div className="flex items-center space-x-2">
                  <Checkbox id="select-all" checked={isAllTeamsSelected} onCheckedChange={handleSelectAllTeams}/>
                ...
                <div className="border-t border-vista-secondary/40 pt-2">
                  {isLoadingTeams ? (<p className="text-vista-light/70 text-sm p-2">Загрузка команд...</p>) : (<div className="space-y-2">
                      {teams.map((team) => (<div key={team.id} className="flex items-center space-x-2">
          ...
          <div className="flex items-center gap-2 mb-4">
            <Button variant={calendarView === 'month' ? 'default' : 'outline'} size="sm" onClick={() => setCalendarView('month')} className="text-xs">
              Месяц
            </Button>
            <Button variant={calendarView === 'week' ? 'default' : 'outline'} size="sm" onClick={() => setCalendarView('week')} className="text-xs">
              Неделя
            </Button>
          </div>
          <div className="flex items-center space-x-2">
            {calendarView === 'month' ? (<>
            <Button variant="outline" size="icon" onClick={handlePreviousMonth} className="border-vista-secondary/50 text-vista-light shadow-sm h-7 w-7">
              <ChevronLeft className="h-3.5 w-3.5"/>
            </Button>
            <div className="text-vista-light font-medium min-w-[140px] text-center text-sm">
              {getFormattedMonth(currentMonth)}
            </div>
            <Button variant="outline" size="icon" onClick={handleNextMonth} className="border-vista-secondary/50 text-vista-light shadow-sm h-7 w-7">
              <ChevronRight className="h-3.5 w-3.5"/>
            </Button>
              </>) : (<>
                <Button variant="outline" size="icon" onClick={handlePreviousWeek} className="border-vista-secondary/50 text-vista-light shadow-sm h-7 w-7">
                  <ChevronLeft className="h-3.5 w-3.5"/>
                </Button>
                <div className="text-vista-light font-medium min-w-[140px] text-center text-sm">
                ...
                <Button variant="outline" size="icon" onClick={handleNextWeek} className="border-vista-secondary/50 text-vista-light shadow-sm h-7 w-7">
                  <ChevronRight className="h-3.5 w-3.5"/>
                </Button>
              </>)}
          </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="w-full">
            ...
            <div className="grid grid-cols-7 gap-1 mb-2">
              {weekDays.map((day, index) => (<div key={index} className="p-2 text-center font-semibold text-vista-light/80 border-b border-vista-secondary/20">
            ...
            return (<div key={index} className={`
                    ...
                    if (training.type === 'MATCH') {
                        return (<div key={training.id} className={`text-xs p-2 rounded-lg border flex flex-col relative cursor-pointer min-h-[110px] ${getTrainingStyles(training)} ${statusClasses[status]} hover:bg-vista-secondary/10 transition-colors hover:shadow-md`} onClick={() => navigateToEvent(training)} style={{ marginBottom: 2 }}>
                              ...
                              <div className="flex items-center justify-between mb-1.5">
                                <div className="flex items-center font-medium text-xs text-vista-light">
                                  ...
                                  {training.type === 'MATCH' && training.competitionType && (<span className="ml-1 text-vista-light/50 text-[10px]">(
                              ...
                              <div className="text-vista-light/90 text-xs font-medium mb-1">
                                <div className="flex items-center w-full">
                                  ...
                                  <span className={`ml-2 px-2 py-0.5 rounded font-semibold text-xs w-7 min-w-[28px] text-center flex items-center justify-center ${getMatchResultClass(training)}`}>{(_a = training.teamGoals) !== null && _a !== void 0 ? _a : '-'}</span>
                                </div>
                                <div className="flex items-center w-full mt-0.5">
                                  <span className="truncate flex-1">{training.isHome ? training.opponentName : training.teamName}</span>
                                  <span className={`ml-2 px-2 py-0.5 rounded font-semibold text-xs w-7 min-w-[28px] text-center flex items-center justify-center ${getMatchResultClass(training)}`}>{(_b = training.opponentGoals) !== null && _b !== void 0 ? _b : '-'}</span>
                                </div>
                              </div>
                    ...
                    return (<div key={training.id} className={`text-xs p-1.5 rounded-md border flex flex-col relative cursor-pointer ${getTrainingStyles(training)} ${statusClasses[status]} hover:bg-vista-secondary/10 transition-colors hover:shadow-md`} onClick={() => navigateToEvent(training)}>
                            <div className="flex items-center justify-between mb-1.5">
