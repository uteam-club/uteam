...
export default function MatchesPage() {
    ...
    return (<div className="space-y-6">
      <Card className="bg-vista-dark/50 border-vista-secondary/50 shadow-md">
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Button className="bg-vista-primary hover:bg-vista-primary/90" onClick={() => setIsAddModalOpen(true)}>
        ...
        <CardContent>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6 items-end">
            ...
            <div className="relative flex-1 md:max-w-[250px]">
              ...
              <Input className="pl-8 bg-vista-dark-lighter border-vista-secondary/30 text-vista-light w-full" placeholder="Поиск матчей..." value={searchValue} onChange={handleSearchChange}/>
            </div>
            ...
            if (dateInput) {
                ...
                <Input id="filter-start-date" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="pl-10 bg-vista-dark-lighter border-vista-secondary/30 text-vista-light focus:border-vista-primary focus:ring-1 focus:ring-vista-primary/50 cursor-pointer [&::-webkit-calendar-picker-indicator]:hidden" placeholder="Дата от" onClick={(e) => {
            try {
                e.target.showPicker();
            }
            catch (error) {
            ...
            if (dateInput) {
                ...
                <Input id="filter-end-date" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="pl-10 bg-vista-dark-lighter border-vista-secondary/30 text-vista-light focus:border-vista-primary focus:ring-1 focus:ring-vista-primary/50 cursor-pointer [&::-webkit-calendar-picker-indicator]:hidden" placeholder="Дата до" onClick={(e) => {
            try {
                e.target.showPicker();
            }
            catch (error) {
            ...
            {hasActiveFilters && (<div className="flex-none">
                <Button variant="ghost" className="w-full md:w-auto h-10 flex items-center text-vista-light/70 hover:text-vista-light" onClick={resetFilters}>
          ...
          {hasActiveFilters && (<div className="mb-4 flex flex-wrap gap-2">
              {selectedTeam && selectedTeam !== "all" && (<Badge className="bg-vista-primary/20 text-vista-light flex items-center gap-1 pl-2">
                  <Users size={12}/>
                  {((_a = teams.find(t => t.id === selectedTeam)) === null || _a === void 0 ? void 0 : _a.name) || 'Команда'}
                  <Button variant="ghost" className="h-5 w-5 p-0 ml-1 hover:bg-vista-primary/30" onClick={() => setSelectedTeam("all")}>
                    <X size={10}/>
                  </Button>
                </Badge>)}
              
              {startDate && (<Badge className="bg-vista-primary/20 text-vista-light flex items-center gap-1 pl-2">
                  <CalendarIcon size={12}/>
                  От: {new Date(startDate).toLocaleDateString('ru-RU')}
                  <Button variant="ghost" className="h-5 w-5 p-0 ml-1 hover:bg-vista-primary/30" onClick={() => setStartDate('')}>
                    <X size={10}/>
                  </Button>
                </Badge>)}
              
              {endDate && (<Badge className="bg-vista-primary/20 text-vista-light flex items-center gap-1 pl-2">
                  <CalendarIcon size={12}/>
                  До: {new Date(endDate).toLocaleDateString('ru-RU')}
                  <Button variant="ghost" className="h-5 w-5 p-0 ml-1 hover:bg-vista-primary/30" onClick={() => setEndDate('')}>
                    <X size={10}/>
                  </Button>
                </Badge>)}
            </div>)}
          ...
          {isLoading ? (<div className="flex justify-center py-10">
            ...
            </div>) : filteredMatches.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredMatches.map(match => {
                var _a, _b, _c, _d;
                return (<div key={match.id} className="p-4 border border-vista-secondary/30 rounded-lg bg-vista-dark-lighter/50 cursor-pointer hover:bg-vista-dark-lighter transition-colors shadow-sm" onClick={() => handleMatchClick(match.id)}>
                  <div className="flex flex-col space-y-3">
                    ...
                    <div className="flex items-center justify-between mt-2 bg-vista-dark/30 p-3 rounded">
                      <div className="w-5/12 text-left">
                        <p className="font-semibold text-vista-light">{match.isHome
                        ? (((_a = match.team) === null || _a === void 0 ? void 0 : _a.name) || ((_b = teams.find(t => t.id === match.teamId)) === null || _b === void 0 ? void 0 : _b.name) || 'Неизвестно')
                        : match.opponentName}</p>
                      </div>
                      ...
                      <div className="w-5/12 text-right">
                        <p className="font-semibold text-vista-light">{match.isHome
                        ? match.opponentName
                        : (((_c = match.team) === null || _c === void 0 ? void 0 : _c.name) || ((_d = teams.find(t => t.id === match.teamId)) === null || _d === void 0 ? void 0 : _d.name) || 'Неизвестно')}</p>
                      </div>
                    </div>
                  </div>
                </div>);
            })}
            </div>) : (<div className="flex flex-col items-center justify-center py-10 gap-4">
      ...
      <AddMatchModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onMatchAdded={fetchMatches}/>
    </div>);
}
