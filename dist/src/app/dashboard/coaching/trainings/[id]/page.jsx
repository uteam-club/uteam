...
import { Calendar, Clock, MapPin, Users, Trash, Save, Plus, ArrowLeft, FileText, Tag, Search, Check, X, ClipboardCheck } from 'lucide-react';
...
export default function TrainingPage() {
    ...
    const loadExercises = async () => {
        ...
        try {
            ...
            const safeData = data.filter(Boolean).map(ex => (Object.assign(Object.assign({}, ex), { category: ex && ex.category && typeof ex.category === 'object' ? ex.category : { id: '', name: 'Без категории' }, author: ex && ex.author && typeof ex.author === 'object' ? ex.author : { id: '', name: 'Неизвестно' } })));
            ...
            const uniqueAuthors = Array.from(new Map(safeData.map((ex) => [ex.author.id, ex.author])).values());
    ...
    useEffect(() => {
        ...
        if (searchQuery) {
            filtered = filtered.filter(ex => ex.title.toLowerCase().includes(searchQuery.toLowerCase()));
        }
        // Фильтр по категории
        if (selectedCategory) {
        ...
        if (selectedTags.length > 0) {
            filtered = filtered.filter(ex => selectedTags.some(tagId => ex.tags.some(tag => tag.id === tagId)));
        }
    ...
    useEffect(() => {
        if (selectedCategory) {
            const tagsForCategory = tags.filter(tag => tag.exerciseCategoryId === selectedCategory);
            setFilteredTags(tagsForCategory);
            // Очищаем выбранные теги, которые не принадлежат этой категории
            setSelectedTags(prev => prev.filter(tagId => tagsForCategory.some(tag => tag.id === tagId)));
        }
        else {
    ...
    const handleAddSelectedExercises = async () => {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error('Ошибка при добавлении упражнений');
    ...
    const handleCompletedChange = async (checked) => {
        try {
            ...
            if (training) {
                const savedData = localStorage.getItem(`training_data_${trainingId}`) || '{}';
                const parsedData = JSON.parse(savedData);
                const updatedData = Object.assign(Object.assign({}, parsedData), { isCompleted: checked });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const trainingData = Object.assign(Object.assign({}, training), { isCompleted });
    ...
    const handleDeleteExercise = (trainingExerciseId) => {
        setLoading(true);
    ...
    const handleDelete = async () => {
        if (!window.confirm('Вы уверены, что хотите удалить эту тренировку? Это действие необратимо.'))
            return;
        setLoading(true);
        try {
            const response = await fetch(`/api/trainings/${trainingId}`, {
    ...
    const updateExerciseOrder = async (newExercises) => {
    ...
    if (error) {
        ...
        <Button variant="outline" onClick={() => router.back()} className="bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20 shadow-sm">
    ...
    return (<div className="space-y-6 pb-8">
      {/* Кнопка возврата */}
      <div>
        <Link href="/dashboard/coaching/trainings" className="inline-flex items-center text-vista-light/70 hover:text-vista-light transition-colors">
        ...
        <CardContent>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-4">
              ...
              {trainingData.location && (<div className="flex items-center bg-vista-dark/70 p-3 rounded-md border border-vista-secondary/50 shadow-md sm:col-span-2 md:col-span-4">
            ...
            {(trainingData.description || trainingData.notes) && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                {trainingData.description && (<div className="bg-vista-dark/30 p-4 rounded-md border border-vista-secondary/30 shadow-md">
                ...
                {trainingData.notes && (<div className="bg-vista-dark/30 p-4 rounded-md border border-vista-secondary/30 shadow-md">
                    <div className="flex items-center mb-2">
                      <FileText className="h-5 w-5 mr-2 text-vista-primary"/>
          ...
          <div>
            ...
            {exercises.length > 0 ? (<div className="space-y-4">
                {exercises.map((exercise, index) => (<div key={exercise.id} className="flex flex-col sm:flex-row rounded-md border overflow-hidden bg-vista-dark/50 border-vista-secondary/50 shadow-md">
                    ...
                    <div className="sm:w-[300px] overflow-hidden bg-vista-dark/30">
                      {exercise.mediaItems && exercise.mediaItems.length > 0 ? (<>
                        ...
                        return imageUrl ? (<div className="w-full h-full flex items-center justify-center p-1">
                                <div className="relative w-full h-full flex items-center justify-center">
                                                                      <img src={imageUrl} alt={exercise.title} className="w-full h-full object-cover" onError={(e) => {
                                e.target.src = `https://picsum.photos/seed/ex${exercise.id}/400/300`;
                                ...
                                exercise.mediaItems[0].type === "VIDEO" && (<div className="absolute inset-0 flex items-center justify-center">
                              ...
                              </div>) : (<div className="w-full h-full flex items-center justify-center text-vista-light/50 min-h-[200px]">
                    ...
                    <div className="flex sm:flex-col items-center gap-2 p-4 border-t sm:border-t-0 sm:border-l border-vista-secondary/50">
                      {/* Кнопка удаления */}
                                              <button className="bg-vista-dark/70 shadow-sm hover:bg-red-600/50 rounded-md p-2 text-vista-light" onClick={(e) => {
                    e.stopPropagation();
                    ...
                    handleDeleteExercise(exercise.trainingExerciseId || '');
                      ...
                      {exercises.length > 1 && (<>
                          <button className={`bg-vista-dark/70 shadow-sm hover:bg-vista-secondary/20 rounded-md p-2 text-vista-light ${index === 0 ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={(e) => {
                        e.stopPropagation();
                        if (index > 0) {
                          ...
                          <button className={`bg-vista-dark/70 shadow-sm hover:bg-vista-secondary/20 rounded-md p-2 text-vista-light ${index === exercises.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={(e) => {
                        e.stopPropagation();
              ...
              </div>) : (<div className="text-center py-10 border border-dashed border-vista-secondary/50 rounded-md">
      ...
      <Dialog open={isExerciseDialogOpen} onOpenChange={setIsExerciseDialogOpen}>
        <DialogContent className="bg-vista-dark/90 text-vista-light border-vista-secondary/50 max-w-screen-lg w-[90vw] max-h-[90vh] overflow-hidden flex flex-col shadow-xl">
          ...
          <div className="space-y-2 mb-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="relative col-span-1 sm:col-span-2 lg:col-span-4">
                ...
                <Input placeholder="Поиск по названию упражнения" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light focus:border-vista-primary focus:ring-1 focus:ring-vista-primary/50"/>
              ...
              {/* Фильтр по автору */}
              <div>
                <Select value={selectedAuthor !== null ? selectedAuthor : "all"} onValueChange={value => setSelectedAuthor(value === "all" ? null : value)}>
                  ...
                  <SelectContent className="bg-vista-dark border-vista-secondary/50 text-vista-light shadow-lg">
                    ...
                    {authors
            .filter((a) => a.id && a.id !== "")
            .map((author) => (<SelectItem key={author.id} value={author.id}>
                          {author.name}
                        </SelectItem>))}
                  </SelectContent>
                </Select>
              ...
              {/* Фильтр по категории */}
              <div>
                <Select value={selectedCategory !== null ? selectedCategory : "all"} onValueChange={value => setSelectedCategory(value === "all" ? null : value)}>
                  <SelectTrigger className="w-full bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light shadow-sm">
            ...
            .map((c) => (<SelectItem key={c.id} value={c.id}>
              ...
              {/* Мультиселект тегов */}
              <div>
                <Select value={selectedTags.length > 0 ? "selected" : "all"} onValueChange={(value) => {
            if (value === "all") {
                setSelectedTags([]);
            }
        }}>
            ...
            setSelectedTags([]);
                      ...
                      {filteredTags.map(tag => (<div key={tag.id} className="flex items-center space-x-2 py-1">
                          <Checkbox id={`tag-${tag.id}`} checked={selectedTags.includes(tag.id)} onCheckedChange={(checked) => {
                if (checked) {
            ...
            {(searchQuery || selectedCategory || selectedAuthor || selectedTags.length > 0) && (<div className="flex flex-wrap items-center gap-2 mt-2">
                <span className="text-vista-light/70 text-sm">Активные фильтры:</span>
                
                {searchQuery && (<Badge className="bg-vista-primary/20 text-vista-primary gap-1 shadow-sm">
                    Поиск: {searchQuery}
                ...
                {selectedCategory && (<Badge className="bg-vista-primary/20 text-vista-primary gap-1 shadow-sm">
                    Категория: {(_a = categories.find((c) => c.id === selectedCategory)) === null || _a === void 0 ? void 0 : _a.name}
                    <button onClick={() => setSelectedCategory(null)}>
                      <X className="h-3 w-3"/>
                    </button>
                  </Badge>)}
                
                {selectedAuthor && (<Badge className="bg-vista-primary/20 text-vista-primary gap-1 shadow-sm">
                    Автор: {(_b = authors.find(a => a.id === selectedAuthor)) === null || _b === void 0 ? void 0 : _b.name}
                    <button onClick={() => setSelectedAuthor(null)}>
                      <X className="h-3 w-3"/>
                    </button>
                  </Badge>)}
                
                {selectedTags.length > 0 && (<Badge className="bg-vista-primary/20 text-vista-primary gap-1 shadow-sm">
                ...
                <Button variant="ghost" size="sm" onClick={resetFilters} className="text-vista-light/70 hover:text-vista-light hover:bg-vista-secondary/20 h-7 px-2">
                  Сбросить все
                </Button>
              </div>)}
          </div>
          ...
          <div className="overflow-y-auto flex-grow" style={{ maxHeight: 'calc(75vh - 200px)' }}>
            {isLoadingExercises ? (<div className="flex justify-center py-12">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-vista-primary"></div>
              </div>) : exercisesError ? (<div className="text-center py-12 border border-dashed border-red-500/50 rounded-md">
                <p className="text-red-400">{exercisesError}</p>
              </div>) : filteredExercises.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {filteredExercises
                .slice((currentPage - 1) * exercisesPerPage, currentPage * exercisesPerPage)
                .map(exercise => (<div key={exercise.id} className={`
                      ...
                      {exercise.mediaItems && exercise.mediaItems.length > 0 ? (<>
                        ...
                        return imageUrl ? (<>
                                <div className="w-full h-full flex items-center justify-center p-1">
                                  <div className="relative w-full h-full flex items-center justify-center">
                                    <img src={imageUrl} alt={exercise.title} className="max-h-full max-w-full object-contain" onError={(e) => {
                                // При ошибке переходим на плейсхолдер
                                e.target.src = `https://picsum.photos/seed/ex${exercise.id}/300/200`;
                    ...
                    <div className="p-2">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1 justify-between">
                        ...
                        {exercise.tags.slice(0, 2).map(tag => (<Badge key={tag.id} className="bg-vista-secondary/20 text-vista-light/90 text-xs">
                            {tag.name}
                          </Badge>))}
                        {exercise.tags.length > 2 && (<Badge className="bg-vista-secondary/10 text-vista-light/70 text-xs">
                            +{exercise.tags.length - 2}
                          </Badge>)}
                      </div>
                    </div>
                  </div>))}
              </div>) : (<div className="text-center py-12 border border-dashed border-vista-secondary/50 rounded-md">
                <p className="text-vista-light/60">
                  Упражнения не найдены
                </p>
                {(searchQuery || selectedCategory || selectedAuthor || selectedTags.length > 0) && (<Button variant="outline" size="sm" onClick={resetFilters} className="mt-4 bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20 shadow-sm">
          ...
          <DialogFooter className="flex justify-between items-center gap-2 mt-3 py-3 flex-wrap sm:flex-nowrap">
            <div className="flex items-center gap-3">
              ...
              {filteredExercises.length > exercisesPerPage && (<nav className="flex items-center gap-1">
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0 bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>
                ...
                if (!isNearCurrent && pageNumber !== 1 && pageNumber !== Math.ceil(filteredExercises.length / exercisesPerPage)) {
                    ...
                    if (pageNumber === currentPage - 2 || pageNumber === currentPage + 2) {
                        return (<span key={pageNumber} className="w-8 h-8 flex items-center justify-center text-vista-light/50">
                            ...
                          </span>);
                    }
                    return null;
                }
                return (<Button key={pageNumber} variant={isCurrentPage ? "default" : "outline"} size="sm" className={`h-8 w-8 p-0 ${isCurrentPage
                        ? "bg-vista-primary text-vista-dark"
                        : "bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20"}`} onClick={() => setCurrentPage(pageNumber)}>
                        <span className="sr-only">Страница {pageNumber}</span>
                        {pageNumber}
                      </Button>);
            })}
                  
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0 bg-vista-dark/70 shadow-sm border-vista-secondary/50 text-vista-light hover:bg-vista-secondary/20" onClick={() => setCurrentPage(p => Math.min(Math.ceil(filteredExercises.length / exercisesPerPage), p + 1))} disabled={currentPage === Math.ceil(filteredExercises.length / exercisesPerPage)}>
                    <span className="sr-only">Следующая страница</span>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-chevron-right"><path d="m9 18 6-6-6-6"/></svg>
                  </Button>
                </nav>)}
              
              {filteredExercises.length > 0 && (<div className="text-vista-light/70 text-sm hidden sm:inline-block">
                  Найдено: {filteredExercises.length}
                  {filteredExercises.length > exercisesPerPage && (<span> (стр. {currentPage} из {Math.ceil(filteredExercises.length / exercisesPerPage)})</span>)}
                </div>)}
            </div>
            ...
            setSelectedExercises([]);
              ...
              <Button size="default" onClick={handleAddSelectedExercises} className="bg-vista-primary hover:bg-vista-primary/90 text-vista-dark shadow-sm" disabled={selectedExercises.length === 0}>
      ...
      <AttendanceModal trainingId={trainingId} isOpen={isAttendanceModalOpen} onClose={() => setIsAttendanceModalOpen(false)}/>
    </div>);
}
