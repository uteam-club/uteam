...
export async function GET(request, { params }) {
    try {
        ...
        const [foundCategory] = await db.select().from(exerciseCategory).where(eq(exerciseCategory.id, categoryId)).limit(1);
        if (!foundCategory) {
...
export async function PUT(request, { params }) {
    try {
        ...
        const [currentCategory] = await db.select().from(exerciseCategory).where(eq(exerciseCategory.id, categoryId)).limit(1);
        if (!currentCategory) {
        ...
        // Обновляем категорию
        const [updatedCategory] = await db.update(exerciseCategory).set(updateData).where(eq(exerciseCategory.id, categoryId)).returning();
        console.log('Категория упражнений успешно обновлена:', updatedCategory === null || updatedCategory === void 0 ? void 0 : updatedCategory.id);
...
export async function DELETE(request, { params }) {
    try {
        ...
        const categoryId = params.id;
        // Получаем текущую категорию
        const [categoryToDelete] = await db.select().from(exerciseCategory).where(eq(exerciseCategory.id, categoryId)).limit(1);
        if (!categoryToDelete) {
            console.log('Категория не найдена:', categoryId);
            return NextResponse.json({ error: 'Exercise category not found' }, { status: 404 });
        }
        ...
        // Удаляем категорию
        const [deletedCategory] = await db.delete(exerciseCategory).where(eq(exerciseCategory.id, categoryId)).returning();
