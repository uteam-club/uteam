...
export async function PUT(req, { params }) {
    try {
        ...
        const [foundTag] = await db.select().from(exerciseTag)
        ...
        const [updatedTag] = await db.update(exerciseTag)
            .set({
            name: data.name,
        ...
        return NextResponse.json({ error: 'Ошибка при обновлении тега упражнений' }, { status: 500 });
    }
}
...
export async function DELETE(req, { params }) {
    try {
        ...
        const clubId = session.user.clubId;
        // Проверяем существование тега и принадлежность к клубу
        const [tagToDelete] = await db.select().from(exerciseTag)
            .where(and(eq(exerciseTag.id, id), eq(exerciseTag.clubId, clubId)));
        if (!tagToDelete) {
            return NextResponse.json({ error: 'Тег не найден' }, { status: 404 });
        }
        // Удаляем тег упражнений
        const [deletedTag] = await db.delete(exerciseTag)
            .where(eq(exerciseTag.id, id))
            .returning();
        // Возвращаем успешный ответ
        return NextResponse.json({ message: 'Тег упражнений успешно удален', deletedTag });
    }
    catch (error) {
        console.error('Ошибка при удалении тега упражнений:', error);
        return NextResponse.json({ error: 'Ошибка при удалении тега упражнений' }, { status: 500 });
    }
}
