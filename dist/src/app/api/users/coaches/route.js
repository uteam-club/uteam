    ...
    try {
        ...
        if (teamId) {
            ...
            const coaches = await db.select({
                ...
                .from(user)
                .where(and(eq(user.clubId, clubId), eq(user.role, 'COACH'), attachedCoachIds.length > 0 ? not(inArray(user.id, attachedCoachIds)) : undefined))
            ...
            const coaches = await db.select({
                ...
                .from(user)
                .where(and(eq(user.clubId, clubId), eq(user.role, 'COACH')))
                .orderBy(asc(user.name));
