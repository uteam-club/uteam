...
export async function GET(request) {
    try {
        ...
        return NextResponse.json({ error: 'Не удалось получить список клубов' }, { status: 500 });
    }
}
...
export async function POST(request) {
    try {
        ...
        const logoUrl = formData.get('logoUrl') || undefined;
        // Проверка обязательных полей
        if (!name || !subdomain) {
            return NextResponse.json({ error: 'Название и поддомен обязательны для заполнения' }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: 'Произошла ошибка при создании клуба' }, { status: 500 });
    }
}
