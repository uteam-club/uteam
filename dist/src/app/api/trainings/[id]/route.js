...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        if ('title' in data && data.title)
            trainingData.title = data.title.trim();
        if ('description' in data)
            trainingData.description = data.description;
        if ('teamId' in data)
            trainingData.teamId = data.teamId;
        if ('date' in data)
            trainingData.date = new Date(data.date);
        if ('location' in data)
            trainingData.location = data.location;
        if ('notes' in data)
            trainingData.notes = data.notes;
        if ('categoryId' in data)
            trainingData.categoryId = data.categoryId;
        if ('status' in data)
            trainingData.status = data.status;
