...
export async function GET(req, { params }) {
    try {
        ...
        const result = links.map(link => {
            ...
            if (!ex)
                return null;
            return Object.assign(Object.assign({}, ex), { position: link.position, trainingExerciseId: link.id, notes: link.notes, tags: tagsByExercise[ex.id] || [], mediaItems: mediaByExercise[ex.id] || [], category: categoryMap[ex.categoryId] || { id: '', name: 'Без категории' }, author: authorMap[ex.authorId] || { id: '', name: 'Неизвестно' } });
        }).filter(Boolean);
