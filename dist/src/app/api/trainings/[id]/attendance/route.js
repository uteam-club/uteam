...
import { training, player, playerAttendance, } from '@/db/schema';
...
async function getTokenFromRequest(request) {
    ...
    try {
        ...
        const decodedToken = jwt.verify(bearerToken, process.env.NEXTAUTH_SECRET || 'fdcvista-default-secret-key-change-me');
        return {
            id: decodedToken.id,
    ...
    try {
        ...
        for (const item of data) {
            ...
            const [existingAttendance] = await db
                .select()
                .from(playerAttendance)
                .where(and(eq(playerAttendance.playerId, item.playerId), eq(playerAttendance.trainingId, trainingId)));
            let result;
            if (existingAttendance) {
                ...
                result = Object.assign(Object.assign({}, existingAttendance), { status: item.status, comment: item.comment || null });
            }
            else {
