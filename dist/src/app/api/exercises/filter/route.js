...
export async function GET(request) {
    ...
    try {
        ...
        const totalCount = ((_b = (_a = totalCountRes.rows) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.count) || 0;
        // ---
        // Пагинация
        ...
        const exercises = exercisesRaw.map(row => {
            ...
            return Object.assign(Object.assign({}, ex), { author: row.User ? { id: row.User.id, name: row.User.name || 'Неизвестно' } : { id: null, name: 'Неизвестно' }, category: row.ExerciseCategory ? { id: row.ExerciseCategory.id, name: row.ExerciseCategory.name } : null, tags: tagsByExercise[ex.id] || [], mediaItems: mediaByExercise[ex.id] || [] });
        });
        // ---
        // Формируем ответ
