...
export const getServiceSupabase = (options) => {
    ...
    console.log('Использую сервисный ключ для Supabase', {
        ...
        timeout: (options === null || options === void 0 ? void 0 : options.timeout) || 'default',
        retries: (options === null || options === void 0 ? void 0 : options.retryCount) || 'default'
    });
    return createClient(supabaseUrl, supabaseServiceKey, {
        ...
        global: {
            ...
            fetch: (url, init) => {
                ...
                return fetch(url, Object.assign(Object.assign({}, init), { signal: controller.signal })).finally(() => clearTimeout(timeoutId));
            }
        }
    });
};
// Функция для установки контекста клуба (для использования с RLS)
export const setClubContext = async (clubId) => {
    try {
...
export async function uploadPlayerFile(file, clubId, teamId, playerId, fileType, documentType // дополнительная метка для типа документа
) {
    try {
        ...
        return {
            ...
            url: data === null || data === void 0 ? void 0 : data.path,
...
export async function deletePlayerFile(filePath) {
    try {
...
export const getDataForClub = async (table, clubId, options = {}) => {
    var _a;
    try {
        ...
        const filters = Object.assign({ clubId }, options.filters);
        ...
        if (options.orderBy) {
            query = query.order(options.orderBy.column, {
                ascending: (_a = options.orderBy.ascending) !== null && _a !== void 0 ? _a : true
            });
        }
        // Добавляем лимит
        if (options.limit) {
