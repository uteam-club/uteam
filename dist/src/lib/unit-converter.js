...
exports.testTimeParsing = testTimeParsing;
...
function parseTimeToSeconds(timeStr) {
    ...
    // Пробуем распарсить как число (может быть в секундах, минутах или часах)
    const numericValue = parseFloat(str);
    if (!isNaN(numericValue)) {
        // Если число меньше 100, считаем что это минуты
        if (numericValue < 100 && numericValue > 0) {
            return numericValue * 60; // минуты в секунды
        }
    ...
    if (parts.length === 3) {
        // hh:mm:ss или mm:ss:ms или ss:ms:ms
        const [first, second, third] = numericParts;
        // Если первое число больше 23, считаем что это mm:ss:ms
        if (first > 23) {
            return first * 60 + second + third / 1000 + milliseconds / 1000;
        }
        // Иначе считаем что это hh:mm:ss
        ...
        // Если значение меньше 60, считаем что это секунды
        if (value < 60) {
            return value + milliseconds / 1000;
        }
        // Если значение меньше 2400 (40 часов), считаем что это минуты
        if (value < 2400) {
            return value * 60 + milliseconds / 1000;
        }
        // Иначе считаем что это уже в секундах
        return value + milliseconds / 1000;
    }
    return NaN;
}
...
function convertUnit(value, fromUnit, toUnit) {
    ...
    if (timeFormats.includes(fromUnit)) {
        ...
        // Избегаем рекурсии, обрабатываем конвертацию напрямую
 ...
 */
function getSupportedUnits(dimension) {
 ...
 */
function isConversionSupported(fromUnit, toUnit) {
    if (fromUnit === toUnit)
        return true;
