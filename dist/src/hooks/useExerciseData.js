...
export function useFilteredExercises(params = {}) {
    ...
    const { data, error, isLoading, mutate } = useSWR(filterUrl, fetcher, {
        revalidateOnFocus: false,
    ...
    return {
        ...
        pagination: (data === null || data === void 0 ? void 0 : data.pagination) || defaultPagination,
        isLoading,
        isError: error,
        mutate,
    };
...
export function useClub(clubId) {
    const { data, error, isLoading } = useSWR(clubId ? `/api/clubs/${clubId}` : null, fetcher, {
        revalidateOnFocus: false,
