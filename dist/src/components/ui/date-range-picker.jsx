...
export function DateRangePicker({ className, dateRange, onDateRangeChange, }) {
    ...
    return (<div className={cn("grid gap-2", className)}>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button id="date" variant="outline" className={cn("w-full justify-start text-left font-normal bg-vista-dark/70 border-vista-secondary/30 text-vista-light", !dateRange && "text-vista-light/70")}>
            ...
            {(dateRange === null || dateRange === void 0 ? void 0 : dateRange.from) ? (dateRange.to ? (<>
                ...
                </>) : (format(dateRange.from, "dd.MM.yyyy", { locale: ru }))) : (<span>Выберите даты</span>)}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0 bg-vista-dark border-vista-secondary/30 shadow-md" align="start">
          <Calendar initialFocus mode="range" defaultMonth={dateRange === null || dateRange === void 0 ? void 0 : dateRange.from} selected={dateRange} onSelect={handleSelect} numberOfMonths={1} locale={ru} classNames={{
