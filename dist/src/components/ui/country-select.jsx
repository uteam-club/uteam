...
export function CountrySelect({ value, onChange, disabled, placeholder = "Выберите страну", className, }) {
    ...
    return (<Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" role="combobox" aria-expanded={open} className={cn("flex h-10 w-full items-center justify-between rounded-md border border-vista-secondary/30 bg-vista-dark/70 px-3 py-2 text-sm text-vista-light ring-offset-vista-dark placeholder:text-vista-light/70 focus:outline-none focus:ring-2 focus:ring-vista-primary/70 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", className)} disabled={disabled}>
          {selectedCountry ? (<div className="flex items-center">
              <img src={`https://flagcdn.com/w20/${selectedCountry.code.toLowerCase()}.png`} alt={selectedCountry.name} className="mr-2 h-4 w-6 object-contain"/>
      ...
      <PopoverContent className="w-[300px] p-0 bg-vista-dark border border-vista-secondary/50 shadow-xl rounded-md overflow-hidden">
        <div className="w-full p-2 border-b border-vista-secondary/30 bg-vista-dark flex items-center">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Поиск страны..." className="w-full bg-transparent border-none text-vista-light text-sm focus:outline-none placeholder:text-vista-light/50"/>
        </div>
        
        <div className="max-h-[320px] overflow-y-auto py-1">
          {filteredCountries.length > 0 ? (<div className="p-1">
              {filteredCountries.map((country) => (<button key={country.code} type="button" onClick={() => handleSelectCountry(country.code)} className={cn("w-full flex items-center text-vista-light py-2 px-3 rounded-md hover:bg-vista-secondary/30 text-left text-sm mb-1", value === country.code && "bg-vista-primary/20 text-vista-primary")}>
                  <img src={`https://flagcdn.com/w20/${country.code.toLowerCase()}.png`} alt={country.name} className="mr-3 h-4 w-6 object-contain"/>
                  <span className="flex-1 truncate">{country.name}</span>
                  {value === country.code && (<Check className="ml-auto h-4 w-4 text-vista-primary"/>)}
                </button>))}
            </div>) : (<div className="text-vista-light py-6 text-center text-sm">
              Страна не найдена
            </div>)}
        </div>
      </PopoverContent>
